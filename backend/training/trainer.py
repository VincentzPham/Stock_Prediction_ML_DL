...
class ModelTrainer:
    ...
    def train_single(
        ...
        **kwargs,
        ...
        Args:
            ...
            use_returns: Use returns-based training (recommended for DL models).
        """
        ...
        try:
            ...
            if mlflow_available:
                active_run = mlflow.start_run(
                    run_name=f"{model_name}{run_name_suffix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
            else:
                active_run = None
            ...
            try:
                ...
                if model_name in DEEP_LEARNING_MODELS:
                    if use_returns:
                        result = self._train_deep_learning_returns(model, preprocessor, **kwargs)
                    else:
                        result = self._train_deep_learning(model, preprocessor, **kwargs)
                elif model_name in TIME_SERIES_MODELS:
                    result = self._train_time_series(
                        model, preprocessor, model_name, **kwargs
                    )
                    ...
                    if mlflow_available and active_run:
                        ...
                        if model.model_dir.exists():
                            mlflow.log_artifacts(
                                str(model.model_dir), artifact_path="model_files"
                            )
    ...
    def _train_deep_learning(
        self, model: BaseModel, preprocessor: DataPreprocessor, 
        multi_horizon: bool = False, **kwargs
    ) -> Dict[str, Any]:
        """Train deep learning models (LSTM, RNN, etc).
        
        Args:
            multi_horizon: If True, train for all horizons at once using Direct Multi-Output.
        """
        if multi_horizon:
            return self._train_deep_learning_multi_horizon(model, preprocessor, **kwargs)
        
        # Single horizon training (legacy)
        ...
        preprocessor.save_scaler(model.MODEL_NAME)
    ...
    def _train_deep_learning_multi_horizon(
        self, model: BaseModel, preprocessor: DataPreprocessor, **kwargs
    ) -> Dict[str, Any]:
        """Train deep learning model with multi-horizon output (Direct Multi-Output)."""
        horizons = PREDICTION_HORIZONS
        
        # Prepare multi-horizon data
        X_train, X_test, y_train, y_test, scaler = preprocessor.prepare_multi_horizon_data(
            horizons=horizons
        )
        
        # Build model with multi-output
        model.build(output_dim=len(horizons))
        model.train(X_train, y_train, X_test, y_test, **kwargs)
        ...
        # Predict
        predictions = model.predict(X_test)  # Shape: (n_samples, len(horizons))
        
        # Inverse transform each horizon
        all_predictions = []
        all_actuals = []
        for i in range(len(horizons)):
            pred_h = scaler.inverse_transform(predictions[:, i].reshape(-1, 1)).flatten()
            actual_h = scaler.inverse_transform(y_test[:, i].reshape(-1, 1)).flatten()
            all_predictions.append(pred_h)
            all_actuals.append(actual_h)
        
        # Evaluate on first horizon (H=1) for primary metric
        metrics = model.evaluate(all_actuals[0], all_predictions[0])
        
        # Add per-horizon metrics
        for i, h in enumerate(horizons):
            h_metrics = model.evaluate(all_actuals[i], all_predictions[i])
        ...
        return {
            "ticker": model.ticker,
            "model": model.MODEL_NAME,
            "predictions": all_predictions[0],  # Primary: H=1
    ...
    def _train_deep_learning_returns(
        self, model: BaseModel, preprocessor: DataPreprocessor, **kwargs
    ) -> Dict[str, Any]:
        """
        Train deep learning model using returns-based approach.
        
        Instead of predicting absolute prices, the model predicts
        cumulative log returns for each horizon. This approach:
        1. Is scale-invariant (works as stock price grows)
        2. Has better statistical properties
        3. Avoids scaler extrapolation issues
        """
        horizons = PREDICTION_HORIZONS
        
        # Prepare returns-based data
        (X_train, X_test, y_train, y_test, 
         returns_scaler, last_prices) = preprocessor.prepare_returns_multi_horizon_data(
            horizons=horizons
        )
        
        if self.verbose:
            print(f"  Returns-based training: {len(X_train)} train, {len(X_test)} test samples")
        
        # Build model with multi-output (one for each horizon)
        model.build(output_dim=len(horizons))
        model.train(X_train, y_train, X_test, y_test, **kwargs)
        
        # Save returns scaler with model
        preprocessor.save_returns_scaler(model.MODEL_NAME)
        
        # Predict returns
        pred_returns = model.predict(X_test)  # Shape: (n_samples, len(horizons))
        ...
        for i, h in enumerate(horizons):
            ...
            actual_returns = y_test[:, i]
            pred_returns_h = pred_returns[:, i]
            
            # Convert returns to prices using last_prices
            pred_prices = preprocessor.returns_to_prices(pred_returns_h, last_prices['test'])
            actual_prices = preprocessor.returns_to_prices(actual_returns, last_prices['test'])
            
            all_predictions.append(pred_prices)
            all_actuals.append(actual_prices)
