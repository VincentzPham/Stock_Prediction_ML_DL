...
from backend.config import MODELS_DIR, RESULTS_DIR, SAVE_PLOTS
...
class BaseModel(ABC):
    ...
    def evaluate(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
        ...
        self.metrics = {"MAE": mae, "MSE": mse, "RMSE": rmse, "MAPE": mape, "R2": r2}
    ...
    def plot_predictions(
        ...
        if save:
            ...
            filepath = (
                self.result_dir
                / f"{self.ticker}_{self.MODEL_NAME}_prediction_{timestamp}.png"
            )
    ...
    def plot_residuals(
        self, actuals: np.ndarray, predictions: np.ndarray, save: bool = SAVE_PLOTS
    ) -> Optional[Path]:
        ...
        plt.suptitle(
            f"{self.ticker} - {self.MODEL_NAME} Residual Analysis", fontsize=14
        )
        plt.tight_layout()
        ...
        if save:
            ...
            filepath = (
                self.result_dir
                / f"{self.ticker}_{self.MODEL_NAME}_residuals_{timestamp}.png"
            )
            plt.savefig(filepath, dpi=300, bbox_inches="tight")
            plt.close()
            return filepath
