...
class ProphetModel(BaseModel):
    ...
    def build(
        ...
        **kwargs
        ...
        self.model = Prophet(
            yearly_seasonality=yearly_seasonality or self.config.get('yearly_seasonality', True),
            weekly_seasonality=weekly_seasonality or self.config.get('weekly_seasonality', True),
            daily_seasonality=daily_seasonality or self.config.get('daily_seasonality', False),
            changepoint_prior_scale=changepoint_prior_scale or self.config.get('changepoint_prior_scale', 0.05)
        )
        
        print(f"Prophet model configured")
    ...
    def plot_components(self, save: bool = True) -> Optional[Path]:
        ...
        if save:
            ...
            filepath = self.result_dir / f"{self.ticker}_{self.MODEL_NAME}_components_{timestamp}.png"
