...
from training.trainer import ModelTrainer, MODEL_REGISTRY
from data.preprocessor import DataPreprocessor
from config import TICKERS, MODEL_NAMES, TARGET_COLUMN, RESULTS_DIR, TIME_STEP
...
def get_trading_days(start_date: pd.Timestamp, num_days: int, ticker: str = 'AAPL') -> List[pd.Timestamp]:
    """
    ...
    try:
        ...
        schedule = calendar.schedule(start_date=start_date + pd.Timedelta(days=1), end_date=end_date)
...
def predict_multi_step_ml(model_instance, preprocessor, num_steps: int) -> List[float]:
    ...
    daily_range_pct = (preprocessor.df['High'] - preprocessor.df['Low']).tail(20).mean() / last_close
...
def predict_multi_step_dl(model, preprocessor, scaler, num_steps: int, time_step: int) -> List[float]:
    """
    ...
    for step in range(num_steps):
        ...
        model_weight = 0.7 * np.exp(-step * 0.03)  # Starts at 0.7, decays to ~0.26 at step 30
...
def predict(request: PredictRequest):
    ...
    try:
        ...
        if not files:
            raise FileNotFoundError(f"No saved model found for {request.ticker} - {request.model}")
        ...
        if model_type == 'deep_learning':
            ...
            for i, (date, pred_price) in enumerate(zip(trading_days, pred_values)):
                predictions_list.append(PredictionDay(
                    day=i + 1,
                    date=str(date.date()),
            ...
            if hasattr(model_instance, 'model') and model_instance.model is not None:
                try:
                    ...
                    if request.model == 'Prophet':
                        future_df = pd.DataFrame({'ds': [d.to_pydatetime() for d in trading_days]})
